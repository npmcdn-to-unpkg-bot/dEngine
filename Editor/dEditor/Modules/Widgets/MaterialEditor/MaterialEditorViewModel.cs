...
using dEditor.Modules.Widgets.ContentBrowser;
using dEditor.Modules.Widgets.MaterialEditor.Nodes;
using dEngine;
...
namespace dEditor.Modules.Widgets.MaterialEditor
{
    public sealed class MaterialEditorViewModel : Document
    {
        ...
        private GuiElement MakePreviewButton(PreviewShape shape, string icon)
        {
            ...
            button.MouseEnter.Connect((x, y) => { button.BackgroundColour = new Colour(0, 0, 0, 0.8f); });

            button.MouseLeave.Connect((x, y) => { button.BackgroundColour = new Colour(0, 0, 0, 0.4f); });
