...
namespace dEngine.Utility
{
    internal static class Noise
    {
        ...
        {
            var curve = new int[tableSize];
            var integral = new int[tableSize];
        ...
        {
            var dev = _intensity*_intensity/4;
            var sat = _saturation*4096/100;

            if (_threadRand == null)
                _threadRand = new Random(Thread.CurrentThread.GetHashCode() ^
                                         unchecked((int)System.DateTime.Now.Ticks));

            var localRand = _threadRand;
